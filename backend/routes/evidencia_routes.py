...
def _verificar_parte_involucrada(escrow_id: str, usuario, db: Session):
    ...
    es_parte = (
        str(billetera_usuario.id) == str(transaccion.billetera_origen)
        or str(billetera_usuario.id) == str(transaccion.billetera_destino)
    )
    if not es_parte:
...
def subir_archivo(
    ...
    _verificar_parte_involucrada(escrow_id, usuario, db)
